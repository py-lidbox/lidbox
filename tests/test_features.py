...
class TestAudio:
    ...
    def test_log10(self):
        for rank in range(1, 5):
            for _ in range(5):
                ...
                y1 = np.log10(x)
                y2 = audio.log10(x.astype(np.float32))
    ...
    def test_power_to_db(self):
        for top_db in range(10, 110, 10):
            for path in audiofiles:
                ...
                dbspec = audio.power_to_db(np.expand_dims(powspec, 0), top_db=float(top_db))[0]
                assert dbspec.numpy().max() <= 0
    ...
    def test_root_mean_square(self):
        for _ in range(100):
            ...
            rms2 = audio.root_mean_square(x, axis=-1)
            assert np.abs(rms1 - rms2.numpy()).max() < 1e-5
