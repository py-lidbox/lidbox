...
@tf.function
def spectrograms(signals, sample_rate, frame_length=400, frame_step=160, power=2.0, fmin=0.0, fmax=8000.0):
...
#TODO energy vad using 0th MFCC index?
...
# requires eager execution for running webrtcvad, can be called from the TF graph with e.g.
# vad_decisions = tf.numpy_function(
#        get_webrtcvad_decisions,
#        [wav_frames, sample_rate, aggressiveness],
#        tf.bool)
# vad_decisions = tf.reshape(vad_decisions, [num_frames])
def get_webrtcvad_decisions(wav_byte_frames, sample_rate, aggressiveness):
    vad = webrtcvad.Vad(aggressiveness)
    is_speech_frames = [vad.is_speech(frame, sample_rate) for frame in wav_byte_frames]
    return tf.constant(is_speech_frames, tf.bool)
