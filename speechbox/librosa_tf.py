...
@tf.function
def spectrograms(signals, n_fft=512, hop_length=160, power=2.0):
    S = tf.math.abs(tf.signal.stft(signals, n_fft, hop_length, fft_length=n_fft))
    return tf.math.pow(S, power)

@tf.function
def melspectrograms(signals, sample_rate, num_mel_bins=40, fmin=60.0, fmax=6000.0):
    S = spectrograms(signals)
